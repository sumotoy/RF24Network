...
void RF24Network::begin(uint8_t _channel, uint16_t _node_address){
	if (!is_valid_address(_node_address)) return;
...
void RF24Network::update(void){
	...
	while (radio.available(&pipe_num)){
		...
		while (!done){
			...
			if (!is_valid_address(header.to_node)) continue;
			// Is this for us?
			...
			#if 0
			// If this was for us, from one of our children, but on our listening pipe, it could mean that we are not listening to them.  If so, open up
...
bool RF24Network::write(RF24NetworkHeader& header,const void* message, size_t len){
	...
	if (len) memcpy(frame_buffer + sizeof(RF24NetworkHeader),message,min(frame_size-sizeof(RF24NetworkHeader),len));
	#ifdef PRINTFENABLED
...
bool RF24Network::write(uint16_t to_node){
	bool ok = false;
	// Throw it away if it's not a valid address
	if (!is_valid_address(to_node)) return false;
	...
	// NOT NEEDED anymore.  Now all reading pipes are open to start.
	#if 0
		// If we are talking on our talking pipe, it's possible that no one is listening. If this fails, try sending it on our parent's listening pipe.  That will wake
		// it up, and next time it will listen to us.
		if (!ok && send_node == parent_node) ok = write_to_pipe(parent_node, 0);
	#endif
...
void RF24NetworkHeader::toString(void)
#else
...
bool RF24Network::is_direct_child(uint16_t node){
	bool result = false;
	// A direct child of ours has the same low numbers as us, and only  one higher number.
	// e.g. node 0234 is a direct child of 034, and node 01234 is a descendant but not a direct child
...
void RF24Network::setup_address(void){
	...
	parent_pipe = i;
	#ifdef SERIAL_DEBUG && defined(PRINTFENABLED)
...
/****************************************************************************/
/*
This is the replace of printf for teensy3, thanks to stevec
http://forum.pjrc.com/threads/23874-My-Kingdom-for-printf%28%29?highlight=printf
*/

#if defined(TEENSY3X)
int RF24Network::kprintf(char *format, ...)  {
#ifdef PRINTFENABLED
...
bool is_valid_address(uint16_t node){
	...
	while(node) {
		...
		if (digit < 1 || digit > 5) {
			...
			#ifdef PRINTFENABLED
				#if defined(TEENSY3X) || defined(ARDUE)
					Serial.print("*** WARNING *** Invalid address 0");
					Serial.print(node);
					Serial.print("\n\r");
				#else
