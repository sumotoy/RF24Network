...
const uint16_t node_address_set[10] = { 
  00, 02, 05, 012, 015, 022, 025, 032, 035, 045 };

uint8_t nodeconfig_read(void)
{
  uint8_t result = 0;

  // Look for the token in EEPROM to indicate the following value is a validly set node address 
  ...
  {
    Serial.print("*** No valid address found.  Send 0-9 via serial to set node address\n\r");
    while(1)
    {
      nodeconfig_listen();
    }
...
void nodeconfig_listen(void)
{
  ...
  if (Serial.available()){
    ...
    if (c >= '0' && c <= '9'){
      ...
      Serial.print("\n\rManually reset index to: ");
      Serial.print(c);
      Serial.print(", address 0");
      Serial.print(node_address_set[c-'0']);
      Serial.print("\n\rPress RESET to continue!");
      while(1);
    }
  }
}
